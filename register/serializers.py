...
from register.models import UserDetails, UserAddresses
...
class RegistrationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UserDetails
        fields = ( 'first_name', 'last_name', 'email', 'username', 'password', 'password2', 'is_active')
        depth = 1
        extra_kwargs = {
...
class UserAddressesSerializer(serializers.ModelSerializer):
    ...
    userdet=RegistrationSerializer(many=True, read_only=True)
    ...
    def create(self, validated_data):
        user = UserAddresses.objects.create(
            useradd = validated_data['useradd'],
    ...
    def validate_UserAddress(self, username):
        existing = UserAddresses.objects.filter(username=username).first()
        if existing:
    ...
    class Meta:
        model = UserAddresses
        fields = ('useradd','userdet', 'address', 'city', 'state', 'country', 'pincode')
